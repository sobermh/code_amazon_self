...
def thread_task(data, pro_file, error_pro_file):
    # 单条数据的处理逻辑
...
def retry_error_data(error_pro_file, pro_file):
    ...
    with ProcessPoolExecutor(max_workers=3) as process_executor:
        # 将数据分割成多个部分，每个进程处理一部分
        chunk_size = max(1, len(error_data) // 3)
        chunks = [error_data[i:i + chunk_size] for i in range(0, len(error_data), chunk_size)]

        # 提交任务到进程池
        for chunk in chunks:
            process_executor.submit(partial(thread_task, chunk, pro_file, error_pro_file))
...
if __name__ == '__main__':
    ...
    print("Time taken:", mid - start, "seconds")
